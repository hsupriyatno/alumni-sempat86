...
if st.session_state.menu_aktif == "Home":
    ...
    if not df_ev_list.empty:
        ...
        if imgs:
            ...
            if not df_k.empty: 
                df_k['waktu_dt'] = pd.to_datetime(df_k['waktu'], errors='coerce')
                df_k['waktu'] = df_k['waktu_dt'].dt.strftime('%d %B %Y')
                st.table(df_k[['waktu', 'nama_penulis', 'isi_komentar']])
    ...
    if not df_mem.empty:
        ...
        for i, row in df_mem.iterrows():
            with cols[i % 3]:
                ...
                # TAMPILAN: Format Tanggal Wafat yang rapi
                try:
                    tgl_wafat_view = pd.to_datetime(row['tanggal_wafat']).strftime('%d %B %Y')
                except:
                    tgl_wafat_view = row['tanggal_wafat']
                
                st.caption(f"Wafat: {tgl_wafat_view}")
...
# --- D. NETWORKING ---
    ...
    st.info("🎯 **Rencana Fitur:** Kolaborasi bisnis antar alumni rekan-rekan SEMPAT 86.")
...
# --- F. ADMIN PANEL ---
    ...
    with t3:
        with st.form("up_mem", clear_on_submit=True):
            ...
            # INPUT: Sekarang menggunakan Kalender (Date Input)
            m_tgl = st.date_input("Tanggal Wafat", value=datetime.now())
            ...
            if st.form_submit_button("Simpan"):
                if m_nama and m_foto:
                    ...
                    conn = sqlite3.connect('alumni.db')
                    conn.execute("INSERT INTO data_memoriam (foto, nama, tanggal_wafat, keterangan) VALUES (?,?,?,?)", (p, m_nama, str(m_tgl), m_ket))
