...
if st.session_state.menu_aktif == "Home":
    # 1. Banner & Kalimat Pembuka (Tetap Ada)
    ...
    # 2. Tambahan Petunjuk & Tombol Navigasi (Agar user HP tidak bingung)
    st.info("💡 **Tips:** Klik tombol di bawah ini untuk menjelajah, atau klik ikon **> (panah)** di pojok kiri atas untuk menu lengkap.")
    ...
    with col_nav3:
        if st.button("🗓️ Cek Agenda Kegiatan", use_container_width=True):
            # Tombol ini membantu user scroll atau sekedar tahu ada menu Agenda
            st.toast("Silakan scroll ke bawah untuk melihat Agenda!") 
    with col_nav4:
        if st.button("⚙️ Masuk Admin Panel", use_container_width=True):
            pindah("Admin Panel")

    st.write("---")
    
    # 3. Bagian Agenda & Dokumentasi (Tetap di bawah)
    st.subheader("🗓️ Agenda Kegiatan (Mendatang)")
    # ... (lanjutkan dengan kode database agenda Bapak seperti biasa)
    conn = sqlite3.connect('alumni.db')
